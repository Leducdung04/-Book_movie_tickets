...
const AmThanhThanhCong = async () => {
  ...
  const ThemBill = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const dulieu=await response.json()
        console.log('Thêm bills thành công',dulieu.data._id );
        return dulieu.data._id
      } else {
        Alert.alert('Thực hiện lỗi vui lòng thử lại')
        return(false)
  ...
  const addTicket=(id)=>{
      Chair_select.forEach(async item=>{
           try {
               ...
               if(response.ok){
                   setanhThanhToan(null)
                   setImgQR(null)
                   console.log("Thêm vé thành công")
               }else{
                Alert.alert('Thực hiện lỗi vui lòng thử lại')
                return false
                   console.log("Failed to add ticket",response.status)
               }
           } catch (error) {
  ...
  const handleSuccess = async () => {
    payMethod==1?setcomfirmBooking(false):setcomfirmBookingByPay(false);
    setCheckSuccess(true);
    await AmThanhThanhCong();
                      ...
                      <View style={{ flexDirection: "row" }}>
                        <View style={{ width:widthScreen/10*3, marginStart: 10 }}>
                          <View style={{ flexDirection: "row", alignItems: 'center', margin: 5 }}>
                            <AntDesign name="QQ" size={30} color='#DDDDDD' />
                            <Text>Ghế trống</Text>
                          </View>
                          <View style={{ flexDirection: "row", alignItems: 'center', margin: 5 }}>
                            <AntDesign name="QQ" size={30} color="#94d0ee" />
                            <Text>Đang chọn</Text>
                          </View>
                          <View style={{ flexDirection: "row", alignItems: 'center', margin: 5 }}>
                            <AntDesign name="QQ" size={30} color="red" />
                            <Text>Đã đặt</Text>
                          </View>
                        </View>
                     ...
                     <View style={{width:'90%',height:'85%',backgroundColor:'white',borderRadius:20,
                      shadowColor: '#000',shadowOffset: { width: 1, height: 2 },shadowOpacity: 0.3,shadowRadius: 3.84,
                               ...
                               <View style={{flex:1,flexDirection:'row',justifyContent:'space-around',alignItems:'center'}}>
                                    ...
                                    <TouchableOpacity onPress={()=>{
                                        if(ImgQR==null){
                                           Alert.alert("Vui lòng chọn ảnh thanh toán ")
                                        }else{handleOrder()}
                                      }}>
                                    <View style={styles.Button}>
                                       <Text style={styles.textButton}>Xác nhận</Text>
